...
| **可以写批注** | 页面任意位置双击就能写便签，能拖动、能换 6 种颜色，自动保存 |
...
| 单击 | 让答案常亮，再点一下盖回去 |
| 顶部「全部显示 / 全部隐藏」 | 一键切换 |
| **双击页面任意位置** | 写一条批注（拖动顶部细条移动，点小圆点换色，× 删除） |
| 点顶部「✎ 画笔」 | 进入画笔模式，按住鼠标画线；面板里有 6 种颜色、3 档粗细、橡皮 |
| **`Ctrl+Z`** | 撤销上一步（画错的一笔、擦掉的一笔、清空的整页都能撤回） |
| 「导出 / 导入」 | 把批注和手绘打包成 JSON 备份，换电脑时恢复 |

批注、手绘、画笔设置都存在浏览器本地（localStorage），按文档分开存，互不干扰。**清理浏览器缓存或换浏览器前，记得点「导出」存个备份。**
