...
| **划选就能补** | 原文漏标的地方，划选一段文字即可涂黑；标错了也能一键放开 |
| **按格式批量操作** | 划选后选「同格式涂黑/取消」，一键处理全文同一种字体字号（比如所有黑体、所有下划线） |
...
页眉页脚（在多数页面同一位置反复出现的文字，比如书名、页码）会被自动剔除，不会混进正文；而第一页的**大标题虽然在页面顶部，但只出现一次，所以不会被误删**。
...
| 单击 | 让答案常亮，再点一下盖回去 |
| 顶部「全部显示 / 全部隐藏」 | 一键切换 |
| **鼠标划选一段文字** | 弹出小菜单：上面会显示选中文字的格式（如「黑体 13.6」）<br>· **涂黑 / 取消涂黑** —— 只管选中的这几个字<br>· **同格式涂黑 / 同格式取消** —— 全文所有相同格式的文字一起处理 |
| **按住 Alt 划选** | 不弹菜单，松手直接生效（选区里过半已遮就放开，否则涂黑） |
| **双击空白处** | 写一条批注（拖动顶部细条移动，点小圆点换色，× 删除） |
| **按住 Ctrl 拖动** | 随手画线，不用切模式（圈重点、划横线最顺手） |
| 点顶部「✎ 画笔」 | 进入正式画笔模式，可一直画；面板里有 6 种颜色、3 档粗细、橡皮 |
| **`Ctrl+Z`** | 撤销上一步 |
...
撤销是**全局的**：涂黑、批注增删、画错的一笔、橡皮擦掉的一笔、清空的整页，按 `Ctrl+Z` 都能一步步退回去。
